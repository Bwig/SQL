...
-- This time, order the rows by last name and first name, in that 
-- order:
...
-- 4b. List last names of actors and 
-- the number of actors who have that last name, but only for names 
...
FROM staff RIGHT JOIN payment ON 
...
-- 
USE sakila;
CREATE VIEW top_five_genres, SUM(payment.amount) AS "Gross" 
FROM category 
JOIN film_category ON (category.category_id=film_category.category_id)
JOIN inventory ON (film_category.film_id=inventory.film_id)
JOIN rental ON (inventory.inventory_id=rental.inventory_id)
